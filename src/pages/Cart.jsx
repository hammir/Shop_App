    ...
    <div>
    {
      ...
      (
        ...
        <div className="lg:w-[70%]">
          {cart.map((item , index)=> (
             <CartItem key={item.id} item={item} itemIndex={item} />
          ))}
      </div>
      ...
      {/* right part */}
       <div className="md:w-[30%] w-full flex flex-col gap-8 justify-between">
         ...
         <div className="mb-20">
          ...
          border-2 border-[#15803d] hover:bg-white hover:text-[#15803d] transition-all duration-300 ease-in">Checkout</button>
         </div>
      ...
      (<div className="w-screen h-[calc(100vh-80px)] flex flex-col gap-6 justify-center items-center">
        ...
        <NavLink to="/" className="bg-[#16a34a] text-white text-md uppercase font-[600] py-3 px-10 rounded-md
        border-[#16a34a] border-2 hover:bg-white hover:text-[#16a34a] ease-in transition-all duration-300">Go to Shop</NavLink>
      </div>)
    }
    </div>
  )
